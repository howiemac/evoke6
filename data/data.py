...
class DataObject(object):
  ...
  def update(self, args):
    ...
    x = [setattr(self, k, v) for k,v in list(args.items()) if k in self._v_fields]
...
#      print "SET====",name,value
      value=self._v_schema[name](value)#create an instance of the relevant type, thus processing value where required
#      print "  TO:",value
  ...
  def __getattribute__(self, name):
    ...
    if value and isinstance(value,REL):
      ...
      object.__setattr__(self,name,value) #bypass DataObject here, as we are not changing anything
    return value 
...
#    ob.update(data)
    for k,v in list(data.items()): # as per update(data) but allow extra data fields
      setattr(ob, k, v) 
  ...
  def all_change(self):
    ...
    changed = dict((i, True) for i in self._v_fields if getattr(self, i) is not None)
  ...
  @classmethod
  @Permit('no way')
  def old_list(cls, asObjects=True, sql='', like={}, isin={}, orderby='', where='' , limit='', pager=-1, pagelength=20, what='*',_debug=False,**criteria):
    ...
    if not sql:
      pairs = ob.quoted_pairs(list(criteria.items())).replace(',',' and ')
      likes = ob.quoted_pairs(list(like.items()), op='like').replace(',', ' and ')
      ...
      ins = ' and '.join('`%s` in %s' % (k,sql_list(v)) for k,v in list(isin.items()))
      sql = "select %s from %s %s %s %s %s" % (what
        , cls.table
        , (pairs or likes or ins or where) and 'where' or ''
        , ' and '.join(i for i in (pairs, likes, ins, where) if i)
        , orderby 
	, limit
        ) 
    if _debug:
 ...
 #         print k,v
        criteria_sql, criteria_args = cls.sql_quoted_pairs(list(criteria.items()), link='and')
        sqlparts.append(criteria_sql)
        sqlargs += criteria_args

      # like
      if like:
        like_sql, like_args = cls.sql_quoted_pairs(list(like.items()), op='like', link='and')
        like_sql=like_sql.replace("%","%%").replace('%%s','%s') # double the % wildcard (but not any %s)
      ...
      for isin_field, isin_values in list(isin.items()):
        if not isin_values: 
          continue
        isin_sql = ' %s  in (%s)' % (isin_field, ', '.join(['%s']*len(isin_values)))
      ...
      whereclauses = (' and '.join(sqlparts)).replace("%","%%").replace('%%s','%s') # double the % wildcard (but not any %s)
      sql = 'select %s from %s %s %s %s %s' % (what, cls.ns_table(), where, whereclauses, orderby, limit)
      sqlargs = tuple(sqlargs)
      try:
        nowt = sql % sqlargs
      #except TypeError: 
      except: 
        print("data.list unmatched arguments. There are %d arguments." % len(sqlargs))
        print(sql)
        print(sqlargs)
...
#    print "parse_orderby not implemented"
    ...
    for part in parts:
      ...
      clauses.append('`%s`%s%s' % (field, direction and ' ' or '', direction))
    return ', '.join(clauses)  

  @classmethod
  def count(cls, like={}, isin={}, orderby='', where='' , limit='',**criteria):
    """return count of data filtered by these criteria
    """
    return cls.list(asObjects=False,like=like,isin=isin,orderby=orderby,where=where,limit=limit,what="count(uid)",**criteria)[0]["count(uid)"]

  @classmethod
  def list_int(cls,item,like={}, isin={}, orderby='', where='' , limit='',**criteria):
    """return list of integers from item (column) filtered by these criteria
    """
    return [int(i[item]) for i in cls.list(False,"",like=like,isin=isin,orderby=orderby,where=where,limit=limit,what="`%s`" % item,**criteria)]

  @classmethod
  def max(cls, item='uid',like={}, isin={}, orderby='', where='' , limit='',**criteria):
    """return max of item (column) filtered by these criteria
    """
    what='max(%s)' % item
    return cls.list(False,"",like,isin,orderby,where,limit,what=what,**criteria)[0][what]

  @classmethod
  def min(cls, item='uid',like={}, isin={}, orderby='', where='' , limit='',**criteria):
  ...
  @classmethod
  def sum(cls, item='uid',like={}, isin={}, orderby='', where='' , limit='',**criteria):
    """return min of item (column) filtered by these criteria
    """
    what='sum(%s)' % item
    return cls.list(False,"",like,isin,orderby,where,limit,what=what,**criteria)[0][what]
 ...
 # export / import
  ...
  def for_export(self,extras=[]):
    ...
    return dict((k,v) for k,v in list(self.__dict__.items()) if k in self._v_fields or k in extras)

  # Dummy methods for leak testing
  @classmethod
  def mundane(self, *a, **k): return 'OK'
...
#    return type(Schema.table.capitalize(), (RemoteSQLDataObject,), data)
    return type(Schema.table.capitalize(), (MassProducedSQLDataObject,), data) #remote disabled for now
