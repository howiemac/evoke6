...
#    print ">>>>>>>>>>>>>>>>TABLES>>>>>>>>>>>>>>>>>",tables
    ...
    self._v_schema=dict([('uid',INT)]+[(k,v[0]) for (k,v) in list(self._v_columns.items())])# this is for use by data.py
...
#      print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>",ks
    ...
    data={
    'table':self.tablesql,
    'columns':",\n".join(("`%s` %s default %s" % (k,v[0]._v_mysql_type,v[1] is None and "NULL" or ('"%s"' % v[1]))) for (k,v) in list(columns.items())),
