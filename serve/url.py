...
class Url:
  ...
  def url(self, method="", classname="", **atts):
    ...
    try:
      ...
      if cls==self.Config.default_class:  
        ...
        u = "%s/%s/%s%s%s" % (prefix,cls.lower(), uid, method and "/" or "", method)
      if atts:
        u += '?' + urlencode(atts)
      return u
    except:
  ...
  def external_url(self,url=""):
    ...
    if not url.startswith('/'):  # we have either a URL relative to this object, or else we already have a full url
      return url
