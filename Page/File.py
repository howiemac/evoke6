  ...
  def edit_file(self,req):
    ...
    if req.name: #we have an update
      ...
      if req.filedata: # replace filedata
        if not self.file_saved(req,replace=True): # save new file under existing uid
      ...
      if req.code and req.code!=self.code: # rename the file
        ...
        os.rename(self.file_loc(),self.file_loc(name=req.code))  #rename the physical file
...
#     so, fix permits
  ...
  def file_saved(self,req,replace=False):
    ...
    if 'filedata' in req:
      ...
      name=req.filename.replace('\\','/').split('/')[-1] # fix MS brain-dead slashes, and strip off path
#      print ">>>>>>>>>>>>>>>>>>>>",req.filename,extension
      if not filedata:
        req.error="please provide a file"
      if not req.error:
        if replace:
          self.delete_file(replace=True) # delete old file (keep self)
          _file=self
        ...
        # return
        req.message='file "%s" %s' % (_file.name,replace and "replaced" or "added")
        return _file
    return None
  ...
  def get_files(self):
    ...
    if not hasattr(self,'files'):
      self.files=self.list(parent=self.uid,kind="file",orderby='seq,uid')
    return self.files  
