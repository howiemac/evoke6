...
class Image(object):
  ...
  def get_width(self,size=''):
    ...
    if w=='?':
      w=safeint(self.get_pref('thumb_size')) #bodge to allow for broken images which refuse to thumnail
    return w
    ...
    if not hasattr(self,'_stage_data'):
      ...
      if len(data)==2:
        if lexists(self.file_loc()):
          data.append('%sx%s' % pim.open(open(self.file_loc(),'rb')).size)
        else:
          data.append('?x?')
        save=True
      elif data[2][0]=='x':
        if lexists(self.file_loc()):
          data[2]='%sx%s' % pim.open(open(self.file_loc(),'rb')).size
        else:
          data[2]='?x?'
        save=True
      if len(data)==3:
        if lexists(self.thumb_loc()):
          data.append('%sx%s' % pim.open(open(self.thumb_loc(),'rb')).size)    
        else:
          data.append('?x?')
        save=True
      elif data[3][0]=='x':
        if lexists(self.thumb_loc()):
          data[3]='%sx%s' % pim.open(open(self.thumb_loc(),'rb')).size
        else:
          data[3]='?x?'
        save=True
#      if len(data)==4:
...
#    req.page='view'
  ...
  def edit_image(self,req):
    ...
    if req.pos: #we have an update
      # replace filedata
      if req.filedata:
        if not self.image_saved(req,replace=True): # save new image under existing uid
...
#       if data['%s_height' % req.size]=='?':
        data['%s_width' % req.size]='' #this will force a re-fetch in get_stage_data()
...
# thumbnails #########################################
  ...
  def thumb_loc(self):
    ...
    return '%ssite/data/%s/%s' % (self.Config.htdocs_filepath,self.thumb_folder(),self.thumb_name())
...
#### add / remove an image ####### 
  ...
  def image_saved(self,req,replace=False):
    ...
    if 'filedata' in req:
      ...
      extension=(imghdr.what('',filedata) or req.filename.split(".")[-1].lower()).replace('jpeg','jpg')
      if not filedata:
      ...
      if not req.error:
        if replace:
          self.delete_image(replace=True) # delete old file (keep self)
          image=self
          image.stage='%s %s' % (self.get_pos(),self.get_size())  # reset stage to remove sizing data from old image
          ...
          size=self.get_images() and (len(self.images)>0) and self.images[-1].get_size() or 'full' 
        ...
        # return
        req.message='image "%s" %s' % (image.text,replace and "replaced: refresh your browser cache to view the replacement" or "added")
        return image
    return None
