...
class Evo(object):
  ...
  def __call__(self,ob,req,wrapper='', gettext=lambda x:x):
    ...
    if not self.path: # first time only.. get the paths
      ...
      self.path='%s%s/evo/%s' % (ob.Config.app_filepath,klass,self.filename)
      ...
      if not lexists(self.path):
        ...
        if not lexists(self.path):
          self.path='%s%s/evo/%s' % (ob.Config.base_filepath,klass,self.filename)
...
#      print("compiling %s : " % self.filename,cob.pyc)
    ...
    res=self.wrap(cob.pyc,ob,req,wrapper) if wrapper else self.evaluate(cob.pyc,ob,req)
    return res
...
#      lib.send_error(inst, sys.exc_info())
      raise EvoSyntaxError("char %s" % p,'evo bytecode',t[max(0,p-40):p],t[p:min(p+40,len(t)-1)])
      ...
      if x:
       ...
       if kind=='expr': #check syntax
         try:#this will break if there are any globals - we are just looking for syntax errors
           ...
           pass
       push(e,indent,kind)
       z='%s%s' % ("+'\\n'+" if (output and (xindent==indent)) else "",s)
       put(z)
